...
impl App {
    ...
    pub(crate) fn draw_search(&mut self, ui: &mut egui::Ui, ctx: &egui::Context, state: &SearchState) {
        ...
        ui.horizontal_wrapped(|ui| {
            ...
            bar_rect = resp.rect;
            if resp.gained_focus() && !self.cfg.history.is_empty() { self.ui.show_hist = true; }
            if resp.changed() && self.search.query.is_empty() { self.ui.show_hist = false; }
            ...
            egui::ComboBox::from_id_salt("cat_cb")
                ...
                .show_ui(ui, |ui| {
                    for &c in CATS {
                        ui.selectable_value(&mut self.search.cat, c.into(),
            ...
            egui::ComboBox::from_id_salt("idx_cb")
                ...
                .width(130.0)
                .show_ui(ui, |ui| {
                    ui.selectable_value(&mut self.net.indexer, "All".into(),
                        RichText::new("All").font(FontId::proportional(fs)));
                    for idx in &self.net.indexers {
                        ui.selectable_value(&mut self.net.indexer, idx.clone(),
            ...
            if !self.search.query.is_empty()
                && ui.add(egui::Button::new(svg_image(SvgIcon::Close, 14.0, self.pal.sub))
                    ...
                    self.search.query.clear(); self.ui.show_hist = false;
                }
        });
        ...
        match state {
            ...
            SearchState::Done => {
                ...
                if self.search.page >= max_p { self.search.page = max_p.saturating_sub(1); }
                ...
                if !typing && ui.input(|i| i.key_pressed(egui::Key::ArrowDown)) {
                    self.ui.selected = Some(self.ui.selected.map_or(0, |s| (s + 1).min(page_n.saturating_sub(1))));
                ...
                if !typing && ui.input(|i| i.key_pressed(egui::Key::D))
                    && self.ui.selected.is_some() { self.ui.detail_open = !self.ui.detail_open; }
    ...
    pub(crate) fn draw_detail_panel(&mut self, ui: &mut egui::Ui) {
        if !self.ui.detail_open || self.ui.tab != Tab::Search { return; }
    ...
    pub(crate) fn draw_history_dropdown(&mut self, ctx: &egui::Context, bar_rect: egui::Rect, fs: f32) {
        if self.ui.show_hist && !self.cfg.history.is_empty() {
            ...
            if let Some(h) = clicked { self.search.query = h; self.ui.show_hist = false; self.do_search(); }
            if let Some(h) = deleted { self.cfg.history.retain(|x| x != &h); save_cfg(&self.cfg); }
            if clear_all { self.cfg.history.clear(); save_cfg(&self.cfg); self.ui.show_hist = false; }
        }
    }
        ...
        egui::Frame::NONE
            ...
            .show(ui, |ui| {
                ...
                ui.horizontal_wrapped(|ui| {
                    ...
                    let dirty = !self.search.f_text.is_empty() || !self.search.f_seed.is_empty()
                        || !self.search.f_size.is_empty() || !self.search.f_year.is_empty()
                        || !self.search.f_trk.is_empty() || self.search.f_hlth != Hlth::All;
                    if dirty {
                        ui.add_space(8.0);
                        if outline_icon_btn(ui, SvgIcon::Close, "Reset", self.pal.red) {
                            self.search.f_text.clear(); self.search.f_seed.clear(); self.search.f_size.clear();
                            self.search.f_year.clear(); self.search.f_trk.clear(); self.search.f_hlth = Hlth::All;
                            self.search.page = 0;
                            self.ui.sel_set.clear(); self.ui.sel_mode = false;
                        }
                    }
                ...
                ui.horizontal_wrapped(|ui| {
                    ...
                    if self.ui.sel_mode {
                        ...
                        if !self.ui.sel_set.is_empty() && ui.add(egui::Button::new(
                    ...
                    for hf in [Hlth::All, Hlth::Hot, Hlth::Good, Hlth::Slow, Hlth::Dead] {
                        ...
                        if ui.add(egui::Button::selectable(on,
                            ...
                            if self.search.f_hlth != hf {
                                ...
                                self.ui.sel_set.clear(); self.ui.sel_mode = false; // filter changed
                            }
                            self.search.page = 0;
                        }
                        ui.add_space(2.0);
