...
impl App {
    pub(crate) fn draw_rss_detail_panel(&mut self, ui: &mut egui::Ui) {
        if self.ui.tab != Tab::Rss {
            return;
        }
        // Render while open OR still fading out (rss_detail drives the flag;
        // reuse detail_anim so both panels share one animation clock).
        let anim = self.ui.detail_anim;
        let open = self.rss.rss_detail.is_some();
        if !open && anim <= 0.0 {
            return;
        }
        // Cache the item so fade-out still has content after rss_detail clears.
        if open {
            if let Some(di) = self.rss.rss_detail {
                if let Some(feed) = self.rss.rss_feeds.get(self.rss.rss_selected) {
                    if let Some(item) = feed.items.get(di).cloned() {
                        self.ui.detail_row = Some(TorrentResult {
                            title: item.title.clone(),
                            magnet_uri: item.magnet.clone(),
                            link: item.link.clone(),
                            tracker: item.tracker.clone(),
                            size: item.size,
                            seeders: item.seeders,
                            peers: item.leechers,
                            category_desc: item.category.clone(),
                            publish_date: item.pub_date.clone(),
                            details: None,
                        });
                    }
                }
        ...
        let Some(_row) = self.ui.detail_row.clone() else {
            return;
        };
        // Items live in the active feed's state; rebuild the index the same
        // way draw_rss does so the panel and table agree.
        let Some(feed) = self.rss.rss_feeds.get(self.rss.rss_selected) else {
            return;
        };
        let items = feed.items.clone();
        let item = match self.rss.rss_detail.and_then(|di| items.get(di).cloned()) {
            Some(it) => it,
            // Fade-out: use the last visible item if rss_detail was cleared.
            None => match &self.ui.detail_row {
                Some(r) => RssItem {
                    title: r.title.clone(),
    ...
    pub(crate) fn draw_rss(&mut self, ui: &mut egui::Ui, _ctx: &egui::Context) {
        ...
        if self.rss.rss_feeds.is_empty() && !self.rss.rss_add_mode {
            ...
            ui.vertical_centered(|ui| {
                ...
                lbl(
                    ui,
                    "Add Jackett Torznab feeds to auto-refresh torrents",
                    pal.dim,
                    fs,
                );
                ui.add_space(20.0);
                egui::Frame::NONE
                    .fill(tint(pal.accent, 12))
                    .corner_radius(10.0)
                    ...
                    .show(ui, |ui| {
                        ...
                        lbl(
                            ui,
                            "Each indexer in Jackett exposes a Torznab API.",
                            pal.sub,
                            fs - 1.0,
                        );
                        lbl(
                            ui,
                            "You can search any indexer and get live results",
                            pal.sub,
                            fs - 1.0,
                        );
                        lbl(
                            ui,
                            "as an auto-refreshed feed — no browser needed.",
                            pal.sub,
                            fs - 1.0,
                        );
                        ui.add_space(12.0);
                        if outline_btn(ui, "+ Add Feed", pal.accent) {
                            self.rss.rss_add_mode = true;
                        }
                    });
            });
            return;
        ...
        ui.horizontal_top(|ui| {
            ...
            ui.vertical(|ui| {
                ...
                if self.rss.rss_add_mode {
                    self.draw_rss_form(ui, None);
                } else if let Some(idx) = self.rss.rss_edit_idx {
                    self.draw_rss_form(ui, Some(idx));
                } else {
                    self.draw_rss_items(ui);
                }
            });
        });
    }
    ...
    pub(crate) fn draw_rss_sidebar(&mut self, ui: &mut egui::Ui) {
        ...
        egui::Frame::NONE
            ...
            .show(ui, |ui| {
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ...
                        if ui
                            .horizontal(|ui| {
                                ...
                                .on_hover_text("Refresh all feeds")
                                .clicked()
                            })
                            .inner
                        {
                ...
                ui.add_space(4.0);
                ui.add(
                    egui::TextEdit::singleline(&mut self.rss.rss_filter)
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                ...
                for i in 0..len {
                    ...
                    let is_sel = self.rss.rss_selected == i
                        && !self.rss.rss_add_mode
                        && self.rss.rss_edit_idx.is_none();
                    let bg = if is_sel {
                        tint(pal.accent, 22)
                    } else {
                        Color32::TRANSPARENT
                ...
                if let Some(i) = sel {
                    ...
                    self.rss.rss_add_mode = false;
                    self.rss.rss_edit_idx = None;
        ...
        egui::Frame::NONE
            ...
            .show(ui, |ui| {
                if wide_btn(ui, "+ Add Feed", pal.accent) {
                    self.rss.rss_add_mode = true;
                    self.rss.rss_edit_idx = None;
    ...
    pub(crate) fn draw_rss_items(&mut self, ui: &mut egui::Ui) {
        ...
        if self.rss.rss_feeds.is_empty() || sel >= self.rss.rss_feeds.len() {
            return;
        }
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if ui
                            ...
                            .inner
                        {
                            self.refresh_feed(sel);
                        }
                        ui.add_space(6.0);
                        if outline_btn(ui, "Edit Feed", pal.sub) {
                            self.rss.rss_edit_idx = Some(sel);
                        }
                    });
                });
                if let Some(e) = &err {
        ...
        if items.is_empty() {
            ...
            ui.vertical_centered(|ui| {
                ...
                lbl(
                    ui,
                    "Click Refresh to fetch the latest torrents",
                    pal.sub,
                    fs,
                );
            });
        ...
        ui.add_space(2.0);
        // Scroll area allows BOTH axes (unlike TableBuilder which hardcodes
        // hscroll off) — so at narrow widths the columns scroll horizontally
            ...
            .show(ui, |ui| {
                            ...
                            body.row(rh, |mut row| {
                                row.col(|ui| {
                                    ...
                                    let cell_resp = ui.interact(
                                        ui.max_rect(),
                                        egui::Id::new(("rsscell", i)),
                                        egui::Sense::click(),
                                    );
                                    if cell_resp.clicked() {
                                        actions.push((i, "detail"));
                                    }
                                ...
                                row.col(|ui| {
                                    ...
                                    ui.label(
                                        RichText::new(
                                            item.size.map(fmt_size).unwrap_or_else(|| "—".into()),
                                        )
                                        .font(FontId::monospace(fs - 0.5))
                                        .color(pal.sub),
                                    );
                                });
                                row.col(|ui| {
                                ...
                                row.col(|ui| {
                                    ...
                                    ui.horizontal(|ui| {
                                        ...
                                        if svg_btn(ui, SvgIcon::Info, "Item details", pal.sub) {
                                            actions.push((i, "detail"));
                                        }
                                        if svg_btn(
                                            ui,
                ...
                for (i, action) in actions {
                    if let Some(item) = items.get(i).cloned() {
                        match action {
                            "detail" => {
                                self.rss.rss_detail = if self.rss.rss_detail == Some(i) {
                                    None
                                } else {
                                    Some(i)
                                };
    ...
    pub(crate) fn draw_rss_item_detail(&mut self, ui: &mut egui::Ui, item: &RssItem) {
        ...
        ui.add_space(10.0);
        egui::Frame::NONE
            .inner_margin(egui::Margin::symmetric(PANEL_MARGIN_X, 0))
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if ui
                            .add(
                                egui::Button::new(svg_image(SvgIcon::Close, 14.0, pal.sub))
                                    .fill(Color32::TRANSPARENT)
                                    .corner_radius(4.0),
                            )
                            .on_hover_text("Close")
                ...
                egui::Grid::new("rss_item_grid")
                    ...
                    .show(ui, |ui| {
                        if let Some(t) = &item.tracker {
                            grid_row(ui, "Tracker", t, pal.accent, &pal, fs);
                        }
                        if let Some(s) = item.size {
                            grid_row(ui, "Size", &fmt_size(s), pal.text, &pal, fs);
                        }
                        if let Some(s) = item.seeders {
                            grid_row(ui, "Seeders", &s.to_string(), seed_col(s), &pal, fs);
                        }
                        if let Some(l) = item.leechers {
                            grid_row(ui, "Leechers", &l.to_string(), pal.red, &pal, fs);
                        }
                        if let Some(d) = &item.pub_date {
                            grid_row(ui, "Published", &time_ago(d), pal.text, &pal, fs);
                        }
                        if let Some(c) = &item.category {
                            grid_row(ui, "Category", c, pal.text, &pal, fs);
                        }
                    });
                ui.add_space(12.0);
                ...
                if wide_icon_btn(ui, SvgIcon::Star, "Save to Favorites", pal.yellow) {
                    let it = item.clone();
                    self.add_fav_from_rss(&it);
                }
            });
    }
    ...
    pub(crate) fn draw_rss_form(&mut self, ui: &mut egui::Ui, edit_idx: Option<usize>) {
        ...
        if is_edit && self.rss.rss_new_cfg.name.is_empty() {
            if let Some(idx) = edit_idx {
                if let Some(feed) = self.rss.rss_feeds.get(idx) {
                    self.rss.rss_new_cfg = feed.config.clone();
                }
            }
        }
        ...
        ui.vertical_centered(|ui| {
            ...
            lbl(
                ui,
                "Connects to a Jackett Torznab indexer endpoint",
                pal.dim,
                fs - 1.0,
            );
            ui.add_space(20.0);
            ...
            labeled_input(
                ui,
                "Indexer:",
                &mut self.rss.rss_new_cfg.indexer,
                RSS_FORM_W,
                "all (Jackett slug)",
                fs,
                pal.dim,
            ...
            labeled_input(
                ui,
                "Query:",
                &mut self.rss.rss_new_cfg.query,
                RSS_FORM_W,
                "empty = latest torrents",
                fs,
                pal.dim,
            );
            ui.add_space(6.0);
            labeled_input(
                ui,
                "Category:",
                &mut self.rss.rss_new_cfg.category,
                RSS_FORM_W,
                "Torznab cat numbers",
                fs,
                pal.dim,
            ...
            ui.horizontal(|ui| {
                if outline_btn(ui, "Cancel", pal.red) {
                    self.rss.rss_add_mode = false;
                    self.rss.rss_edit_idx = None;
                }
                ui.add_space(12.0);
                if wide_btn(ui, "Save", pal.accent) {
                    if is_edit {
                        if let Some(idx) = edit_idx {
                            self.rss.rss_feeds[idx].config = self.rss.rss_new_cfg.clone();
                        }
                    } else {
                        self.rss
                            .rss_feeds
                            .push(RssFeedState::new(self.rss.rss_new_cfg.clone()));
                        self.rss.rss_selected = self.rss.rss_feeds.len() - 1;
                    }
                    self.sync_rss_configs();
                    self.rss.rss_add_mode = false;
